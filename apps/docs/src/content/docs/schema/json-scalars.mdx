...
- **Variables inside literals** — a variable used within a JSON literal resolves to its value

That last point means all three of these forms work:

```graphql
mutation Save($meta: JSON, $tags: JSON) {
  inline:    insertIntoArticleSingle(values: { title: "A", metadata: { tags: ["x"] } })  { id }
  whole:     insertIntoArticleSingle(values: { title: "B", metadata: $meta })            { id }
  embedded:  insertIntoArticleSingle(values: { title: "C", metadata: { tags: $tags } })  { id }
}
```
