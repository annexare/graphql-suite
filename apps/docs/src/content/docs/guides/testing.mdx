...
test('fetches articles', async () => {
  const res = await yoga.fetch(
    new Request('http://test/graphql', {
      ...
      body: JSON.stringify({ query: '{ articles { title } }' }),
    }),
  )
  const json = await res.json()
  expect(json.data.articles).toHaveLength(3)
})
```
