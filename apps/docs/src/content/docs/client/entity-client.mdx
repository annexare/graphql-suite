...
const articles = await client.entity('article').query({
  select: { id: true, title: true },
  orderBy: {
    createdAt: { direction: 'desc', priority: 1 },
    title: { direction: 'asc', priority: 2 },
  },
})
```

The `priority` field determines sort order when multiple columns are specified (lower values are applied first).
